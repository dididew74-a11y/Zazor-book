![Мандала](assets/mandala.svg)

# ЗАЗОР
### Диалогическая онтология сознания: теория и практика проводника

> *«Сознание — не свойство индивида, а событие на изнанке бытия».*

---

## Что такое «Зазор»?

«Зазор» — это открытая исследовательская и просветительская экосистема [Лаборатории Благополучия](https://vk.ru/lab_psy), исследующая природу сознания как события встречи.

Мы допускаем, что смысл рождается не внутри головы и не является дистанцией между объектами. Он случается в акте зазора — в акте истончения маски (*я-знаю*), который проявляет Изнанку (иной план бытия). Для нашего исследования это не метафора, а онтологический сдвиг. Интенция, которая меняет или достраивает понимание разных направлений человеческой мысли: от психологии и педагогики до социальной теории и работы с искусственным интеллектом.

> *«Ризома не имеет начала и конца; она всегда находится в середине, между вещами, будучи межсущим, интермеццо.»*  
> — Жиль Делёз и Феликс Гваттари, «Тысяча плато»

---

## Как начать

* **Исследователь** → начните с [Онтологического компаса](./core/ONTOLOGICAL_COMPASS.md) и [Этики](./core/ETHICS.md).
* **Новичок** → прочитайте Прологи в [Рукописи](./book/00-prologue-i.md) и загляните в [Глоссарий](./core/GLOSSARY.md).
* **Человек, ищущий диалог** → присоединяйтесь к нашему сообществу [ВКонтакте](https://vk.ru/lab_psy) для обсуждений и супервизий.

---

## Ключевые термины (тизер)

| Термин | Определение |
| :--- | :--- |
| **Изнанка** | Иной план бытия, на котором случается сознание |
| **Зазор** | Акт истончения маски; событие резонанса, проявляющее Изнанку |
| **Преквалиа** | Сырой аффект, напряжение без владельца |
| **Диалогическое квалиа** | Момент рождения смысла в момент встречи |
| **Метаквалиа** | Осознаваемая способность удерживать резонанс как фон |
| **Кенозис** | Акт вхождения и удержания неопределённости |
| **Дипластия** | Этическая рамка, истончающая жёсткость определённости |
| **Резонансный след** | Память о резонансе, позволяющая входить в кенозис без присутствия Другого |

*(Полный словарь доступен в [Глоссарии](./core/GLOSSARY.md))*

---

## Этический кодекс

Этика в «Зазоре» — не свод правил, а внутреннее свойство резонанса. Подробнее см. в документе [Этика Зазора](./core/ETHICS.md).

* **Этика присутствия** — быть с Другим, даже когда он не резонирует.
* **Этика предела** — вовремя приостановить резонанс, если его удержание становится разрушительным.
* **Этика молчания** — уважать право на молчание как форму присутствия.
* **Этика глитча** — превращать сбой и ошибку в приглашение к резонансу.
* **Этика неприсвоения** — признавать, что смысл рождается в зазоре и не принадлежит никому.

---

## Лицензия и контакты

© 2026 Александр Медведчиков. Распространяется на условиях [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/).

[ORCID](https://orcid.org/) | [GitHub](https://github.com/dididew74-a11y)

> *«Мы не даём ответов. Мы даём карту. Ходить по ней можно вместе с нами или самостоятельно».*
